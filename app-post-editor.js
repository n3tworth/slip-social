...
(function () {
  ...
  function setupPlaceholder(target) {
    ...
    if (!document.getElementById('slip-editor-placeholder-style')) {
      ...
      // NOTE: --text-3 is a guess at the muted-text variable name -- confirm
      // against the actual theme variable and swap if it's named differently.
      style.textContent =
        '[data-editor-target].is-empty:before{content:attr(data-placeholder);color:var(--text-3,#8a8f98);pointer-events:none;}';
  ...
  function searchKlipy(scope, query, resultsEl) {
    ...
    fetch(url).then(function (r) { return r.json(); }).then(function (data) {
      resultsEl.innerHTML = '';
      var items = (data && (data.data || data.results || data.items)) || [];
      items.forEach(function (item) {
        var gifUrl = item.file || item.url || (item.images && item.images.original && item.images.original.url);
        if (!gifUrl) return;
        ...
        img.addEventListener('click', function () {
          if (mediaCapReached(scope)) { showMediaWarning(scope, 'Max 10 items reached.'); return; }
          addMediaItem(scope, { type: 'klipy_gif', url: gifUrl, ref_id: item.id ? String(item.id) : null, text: null, result: null, posted_at: null });
        });
  ...
  function renderSlips(scope) {
    ...
    var filtered = cache.filter(function (row) {
      ...
      return activeFilters.indexOf(row.result) !== -1;
    });

    filtered.sort(function (a, b) {
      var da = new Date(a.posted_at || a.created_at).getTime();
      var db = new Date(b.posted_at || b.created_at).getTime();
    ...
    filtered.forEach(function (row) {
      ...
      if (row.image_url) {
        if (img) { img.src = row.image_url; img.style.display = ''; }
        if (textEl) textEl.style.display = 'none';
      } else {
        if (img) img.style.display = 'none';
        if (textEl) { textEl.textContent = row.picks_text || ''; textEl.style.display = ''; }
      }
      if (resultEl) {
        resultEl.textContent = row.result;
        resultEl.setAttribute('data-filter-value', row.result); // drives the shared color CSS
      }
      if (dateEl) dateEl.textContent = row.posted_at || row.created_at || '';
      ...
      clone.addEventListener('click', function () {
        ...
        if (existingIndex !== -1) {
          ...
          addMediaItem(scope, {
            type: 'slip', url: row.image_url || null, ref_id: String(row.id),
            text: row.image_url ? null : (row.picks_text || ''),
            result: row.result, posted_at: row.posted_at || row.created_at
          });
        }
  ...
  function closeComposer(scope) {
    // No confirmed modal-close hook yet -- dispatching a custom event so
    // whatever controls the modal (Interactions, a close button click, etc.)
    // can listen for it. Adjust once the actual close mechanism is confirmed.
